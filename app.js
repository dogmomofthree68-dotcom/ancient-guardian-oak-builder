...
function freshState() {
  ...
  for (let i = 1; i <= TOTAL_LAYERS; i++) {
    layers[i] = { cells: (i === 1 || i === 2) ? layerOneCells() : i === 3 ? layerThreeCells() : i === 4 ? layerFourCells() : i === 5 ? layerFiveCells() : {}, completed: false, notes: "" };
  }
  return {
    currentLayer: 1,
    selected: null,
...
$("resetLayer").addEventListener("click", () => {
  ...
  state.layers[state.currentLayer].cells = state.currentLayer <= 2 ? layerOneCells() : state.currentLayer === 3 ? layerThreeCells() : state.currentLayer === 4 ? layerFourCells() : state.currentLayer === 5 ? layerFiveCells() : {};
  state.selected = null;
  save(); renderAll();
});
...
function previewGeometry() {
  const faces = [];
  const selectedLayer = Math.min(state.currentLayer, 5);
...
function drawPreview() {
  ...
  ctx.fillText(`Layers 1–${Math.min(state.currentLayer,5)} • drag to rotate`,12,20);
}
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", async () => {
    try {
      await navigator.serviceWorker.register("sw.js?v=21", { updateViaCache: "none" });
    } catch (error) {
